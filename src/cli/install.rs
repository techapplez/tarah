...
pub fn tarah_install_pkg(packs: &[String], debug: bool) {
    ...
    for result in results {
        match result {
            Ok(PackageSource::Official(pkg)) => {
                printuse_pacman.push(pkg);
            }
            Ok(PackageSource::Aur(pkg)) => {
                printuse_aur.push(pkg);
            }
            Ok(PackageSource::NotFound(pkg)) => {
                not_found_pkgs.push(pkg);
            }
    ...
    for pack in &not_found_pkgs {
        println!("Package {pack} found nowhere :(");
    }
    ...
    if !printuse_aur.is_empty() {
        ...
        let aur_install_results: Vec<Result<String, (String, String)>> = printuse_aur
            .par_iter()
            .map(|pack| {
                let package_url = format!("https://aur.archlinux.org/{pack}.git");
                ...
                if let Err(e) = fs::create_dir_all(&clone_path) {
                    ...
                    eprintln!("{}", format!("[{pack}] Error: {err_msg}").red());
